...
export default class CandyH extends Component {
    ...
    renderHeader() {
        return (
            <LinearGradient colors={[Colors.mainTab, Colors.LightG]} start={{ x: 0, y: 0.3 }} end={{ x: 0, y: 1 }} style={Styles.totalDiamond}>
                <View style={{ flex: 1, alignItems: 'center' }}>
                    <Text style={Styles.totalNum}>
                        {`${this.state.baseCandyH.toFixed(2)} + ${this.state.extCandyH.toFixed(2)}`}
                    </Text>
                    <View style={{ marginTop: 8, flexDirection: 'row', alignItems: 'center' }}>
                        <FontAwesome name="fire" color={Colors.White} size={24} />
