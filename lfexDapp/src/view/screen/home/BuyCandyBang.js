...
export default class BuyCandyBang extends Component {
   ...
   */
    ...
    render() {
        ...
        let { order } = this.state;
        return (
            <LinearGradient style={{ flex: 1 }} colors={[Colors.mainTab, Colors.LightG]} start={{ x: 0, y: 1 }} end={{ x: 0, y: 0 }}>
                <Header title="分享排行榜" rightText="规则" onRightPress={() => this.onRightPress()} />
                <LinearGradient colors={[Colors.mainTab, Colors.LightG]} start={{ x: 1, y: 0 }} end={{ x: 1, y: 1 }} style={Styles.sequence}>
