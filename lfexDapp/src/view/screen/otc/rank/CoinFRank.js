...
export default class CoinFRank extends Component {
    constructor(props) {
        super(props);
        this.state = {
        };
    ...
    getHomeRankList = () => {
        Coin.findCoinRank({ type: 2, coinType: 1 })
            .then((data) => {
                this.setState({
                    data: data,
                    refreshState: false
    ...
    onPress(item) {
        ...
        Actions.push('Otc', { title: 'fabi', type: item.name });
    }
        ...
        return (
            <TouchableOpacity onPress={() => this.onPress(item)} key={index} style={{ height: 60, flexDirection: 'row', borderBottomWidth: 1, borderBottomColor: Colors.C13, marginLeft: 10, }}>
                ...
                <View style={{ flex: 2, flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
                    <View style={{ flex: 1 }}>
                        <Text style={{ fontSize: 10, color: Colors.C10 }}><Text style={{ fontSize: 12, fontWeight: 'bold', color: Colors.C12 }}>{item.name}</Text>/USDT</Text>
