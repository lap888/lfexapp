...
import { View, Text, NativeModules, StyleSheet, ScrollView, TouchableWithoutFeedback, Linking, Image, Platform, TouchableOpacity, StatusBar, Pressable, DeviceEventEmitter } from 'react-native';
...
class MineScreen extends Component {
	...
	renderHeader() {
		...
		return (
			<View style={{backgroundColor: Colors.main, paddingHorizontal: 20, paddingVertical: 10}}>
				...
				<View>
					...
					<Text style={{fontSize: 12, color: Colors.White, marginTop: 10}}><Text style={{fontSize: 16}}>{false ? 1234123 :'———'}</Text>  {false ? 12341234 :'_ _ _'}</Text>
				</View>
		...
		return (
			<View style={{flex: 1}}>
				<ScrollTopBar
                    ...
                    >
                        <LegalCoin eyes={this.state.eyes} isFocus={this.state.index === 0} />
                        <Currency eyes={this.state.eyes} isFocus={this.state.index === 1} />
                    </ScrollTopBar>
			</View>
		)
	}
	render() {
		return (
			<View style={{ flex: 1 }}>
				...
				{/* {Platform.OS != 'android' ? <View style={{height: 20, width:10}} />  : null} */}
				{this.renderHeader()}
				{this.renderList()}
			</View>
		);
